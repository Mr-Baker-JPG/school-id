...
function createGoogleUser(code?: string | null) {
	...
	return {
		...
		profile: {
			...
			photos: [
				{
					value: 'https://lh3.googleusercontent.com/a/default.jpg',
				},
			],
		},
...
export async function deleteGoogleUser(email: string) {
	...
	if (!user) return null
	await setGoogleUsers(users.filter((u) => u.email !== email.toLowerCase()))
	return user
}
...
export async function insertGoogleUser(code?: string | null, email?: string) {
	...
	if (user) {
		...
		if (email) {
			...
			if (user.profile.emails && user.profile.emails[0]) {
				user.profile.emails[0].value = email
			}
		}
...
async function getUser(request: Request): Promise<GoogleUser | Response> {
...
export const handlers: Array<HttpHandler> = [
	http.post('https://oauth2.googleapis.com/token', async ({ request }) => {
		...
		return json({
			...
			token_type: 'Bearer',
			expires_in: 3600,
			refresh_token: `${user.code}_mock_refresh_token`,
		})
	}),
	http.get(
