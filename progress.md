...
**Feature:** Add to Wallet (Apple Wallet / Google Pay)
...
- Added "Add to Wallet" button to employee ID page (`/employee/id`):
  - Button appears for authenticated employees
  - Positioned next to PDF download button
  - Available on both desktop and mobile views
  - Auto-detects device and generates appropriate pass format
...
- `app/utils/device-detection.server.ts` - Device detection utility
- `app/utils/wallet-pass.server.ts` - Wallet pass generation service
- `app/routes/employee/id/wallet.tsx` - Wallet pass download route
- `app/utils/device-detection.server.test.ts` - Device detection tests
- `app/utils/wallet-pass.server.test.ts` - Wallet pass generation tests
- `app/routes/employee/id/wallet.test.ts` - Wallet pass route tests

**Files Modified:**

- `app/routes/employee/id.tsx` - Added "Add to Wallet" button
- `package.json` - Added `passkit-generator` dependency

**Configuration:**

- Apple Wallet requires environment variables:
  - `APPLE_WALLET_WWDR_CERT` - WWDR certificate (base64 encoded)
