...
test('Student can download their own PDF ID', async () => {
...
test('Student cannot view other students PDFs', async () => {
	// Create user with email that doesn't match any student
	...
	// Should get 404 - student record not found for this user's email
	await expect(
		loader({
			request,
			params: {},
...
test('Unauthenticated users are redirected to login', async () => {
	const request = new Request('http://localhost/resources/student-pdf')

	// Should throw because requireUserId will fail
	await expect(
		loader({
			request,
			params: {},
